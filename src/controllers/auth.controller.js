...
exports.userLogin = async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne(isEmail(username) ? { email: username } : { username })
      .populate('roles', '-__v')
      .populate({
        path: 'savedPost'
      })
    ...
    if (!isValid) return res.status(422).json({ error: 'password is invalid' })
...
exports.userRegister = async (req, res) => {
  ...
  try {
    ...
    let img_thumb
    gender === 'male'
      ? (img_thumb = 'https://res.cloudinary.com/dfsdanimaulana/image/upload/v1661665020/thumbnails/male_avatar_kzr9sl.png')
      : (img_thumb =
